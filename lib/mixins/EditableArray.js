export default {
  ...
  computed: {
    ...
    readonlyItemSchema() {
      if (!this.fullSchema.items || (!this.fullSchema.items.properties && !this.fullSchema.items.oneOf)) {
        console.log(this.fullSchema)
        console.error('readonlyItemSchema() | returning nothing')
        return
      }
      ...
      if (this.itemTitle) {
        if (this.fullSchema.items.properties) {
          delete schema.properties[this.itemTitle]
        } else if (this.fullSchema.items.oneOf) {
          schema.oneOf.forEach((value) => {
            delete value.properties[this.itemTitle]
          })
        }
      }
      return schema
  ...
  methods: {
    renderArrayItemModal(h, item, i) {
      ...
      const scopedSlots = { activator: () => h('v-btn', {
        ...
        h('v-icon', isNew ? this.fullOptions.icons.add : this.fullOptions.icons.configure)
      ]) }
      const close = () => {
        ...
        h('v-card', [
          ...
          h('v-card-actions', [
            ...
            h('v-btn', { props: { text: true },
              on: { click: close },
              attrs: { id: this.fullOptions.idPrefix + this.dashKey + '-dialog_cancel_button' }
            }, 'cancel'),
            h('v-btn', { props: { color: 'primary' },
              ...
              attrs: { id: this.fullOptions.idPrefix + this.dashKey + '-dialog_ok_button' }
            }, 'ok')
          ])
        ])
      ])
        ...
        const listItems = this.value.filter(item => !!item).map((item, i) => h('v-flex', { props: { xs12: true } }, [
          h('v-card', { class: 'my-2', props: { tile: true } }, [
            ...
            h('v-card-actions', [
              ...
              (!this.disabled && !this.fromUrl && !this.fullSchema.fromData) && h('v-btn', { props: { icon: true, color: 'grey' },
                attrs: { id: this.fullOptions.idPrefix + this.dashKey + '--' + i + '-delete_button' },
                on: { click: () => {
